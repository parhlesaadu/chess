...
void YourNextMove(char chessboard[][8], int player) {
	...
	int r, c;
	cout << "Enter the current row/column of the piece you wish to move: ";
	cin >> r >> c;
	while (r < 0 || r > 7 || c < 0 || c > 7 || chessboard[r][c] != piece) {
		cout << "INVALID POSITION! Kindly reenter: ";
		cin >> r >> c;
	}

	int move, validOptions[45], index = 0;
	int* movePTR = &move;

	switch (option) {
		case 1: {
			cout << "Possible moves for selected piece in rows and columns are given below:\n";
			if (r >= 1 && c >= 1 && chessboard[r - 1][c - 1] == '.') {
				cout << "Press 1 for [" << r - 1 << "][" << c - 1 << "]\n";
				validOptions[0] = 1;
			}

			if (r >= 1 && chessboard[r - 1][c] == '.') {
				cout << "Press 2 for [" << r - 1 << "][" << c << "]\n";
				validOptions[1] = 2;
			}

			if (r >= 1 && c < 7 && chessboard[r - 1][c + 1] == '.') {
				cout << "Press 3 for [" << r - 1 << "][" << c + 1 << "]\n";
				validOptions[2] = 3;
			}

			if (c >= 1 && chessboard[r][c - 1] == '.') {
				cout << "Press 4 for [" << r << "][" << c - 1 << "]\n";
				validOptions[3] = 4;
			}

			if (c < 7 && chessboard[r][c + 1] == '.') {
				cout << "Press 5 for [" << r << "][" << c + 1 << "]\n";
				validOptions[4] = 5;
			}

			if (r < 7 && c >= 1 && chessboard[r + 1][c - 1] == '.') {
				cout << "Press 6 for [" << r + 1 << "][" << c - 1 << "]\n";
				validOptions[5] = 6;
			}

			if (r < 7 && chessboard[r + 1][c] == '.') {
				cout << "Press 7 for [" << r + 1 << "][" << c << "]\n";
				validOptions[6] = 7;
			}

			if (r < 7 && c < 7 && chessboard[r + 1][c + 1] == '.') {
				cout << "Press 8 for [" << r + 1 << "][" << c + 1 << "]\n";
				validOptions[7] = 8;
			}
		...
		case 3: {
			...
			for (int i = 1; i <= 8; i++) {
				if (c != i - 1 && (chessboard[r][i - 1] < b1 || chessboard[r][i - 1] > b2)) {
					cout << "Press " << i << " for [" << r << "][" << i - 1 << "]\n";
					validOptions[index] = i; index++;
				}
			}
			for (int i = 1; i <= 8; i++) {
				if (r != i - 1 && (chessboard[i - 1][c] < b1 || chessboard[i - 1][c] > b2)) {
					cout << "Press " << i + 8 << " for [" << i - 1 << "][" << c << "]\n";
